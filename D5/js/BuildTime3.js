...
function searchJob(jobPosition,where){
  
  for(i=0; i<jobs.length; i++){
    if(jobs[i].title.includes(jobPosition)||jobs[i].location.includes(where)){  //FILTRO DI RICERCA PER PAROLE DA INSERIRE NEGLI ARGOMENTI DELLA FUNZIONE COME STRINGHE
      searchResult.push("position: "+jobs[i].title+"  ---->  location: "+ jobs[i].location);  // PUSH DEI RISULTATI NELL'ARRAY SEARCHRESULT
    }
  }
  return searchResult;
...
console.log(searchJob("Dev","NY"));
console.log("number of results: "+searchResult.length); //MOSTRA IN CONSOLE IL NUMERO DI RISULTATI OTTENUTI